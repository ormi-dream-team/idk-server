...
public class AnswerService {
    ...
    public void create(Provider provider, Answer answer) {
        throwIfAnswerExists(answer.getQuestion(), provider);
    ...
    void throwIfAnswerExists(Question question, Provider user) {
        if (answerRepository.existsByQuestionIdAndUserId(question.getId(), user.id())) {
    ...
    boolean isAdmin (Provider provider) {
        return provider.roles().contains(ADMIN);
    }
